...
process.title = "Multithread AVAX C-CHAIN Bruteforce by CorvusCodex";
...
data.toString().split("\n").forEach(address => {
    if (address.startsWith('0x')) {
        ...
        console.error('Error: AVAX C-CHAIN addresses are not in correct format. Addresses must start with 0x');
        process.exit(1);
    }
});
...
function generate() {
    ...
    // Generating a new random AVAX C-CHAIN keypair
